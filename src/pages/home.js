...
function Home() {
  ...
  const covertMinMaxVal = (val) =>
    val.split("_")[0] + " " + val.split("_")[1] + " = " + val.split("_")[2];
  ...
  const mergeData = (jsonData) => {
    ...
    for (let i = 0; i < uniqueData.length; i++) {
      const data = jsonData?.fields?.filter(
        (obj) => obj.section === uniqueData[i]
      );
      if (data.length > 0)
        sectionData.push({
          section: uniqueData[i],
          data: data,
          sectionName: data[0].section_name,
        });
    }
  ...
  return (
    <section className="p-4 mt-3">
      <div className="container mb-4">
        ...
        {formData?.fields?.map((sectionData, index) => (
          <div key={index}>
            <Caption name={sectionData.sectionName} />
            {sectionData?.data?.map((el, index) => (
              <div key={index} className="row mt-5">
                ...
                <div className="col-5">
                  ...
                  {el.field_type === "radio" &&
                    el.field_options.map((radioData, index) => (
                      <div key={index} className="form-check form-check-inline">
                        ...
                        <Label
                          ...
                          forhtml={`${el.field_id}-${index}`}
                        />
                      </div>
                    ))}
                  {el.field_type === "select" && (
                    <Select
                      classname="form-select"
                      optionData={el.field_options}
                      labelforSelected="Select Education..."
                    />
                  )}
                </div>
                <div className="col-2 text-end">
                  <span className="form-text">
                    <Href
                      ...
                      dataToolTipHtml={`<b>${el.info}</b> <br/> ${
                        el.field_id === "completed_year"
                          ? el.validations[0] +
                            " and " +
                            covertMinMaxVal(el.validations[1]) +
                            ", " +
                            covertMinMaxVal(el.validations[2])
                          : el.validations.join(" and ") + " only"
                      } `}
